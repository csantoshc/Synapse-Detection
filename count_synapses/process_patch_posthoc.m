...
function [Bigpatch,Patch] = process_patch_posthoc(Patch,Segment,Bigpatch,croppix)
...
Bigpatch = (Bigpatch-mean(Bigpatch(:))) ./ std(Bigpatch(:));  
...
Bigpatch = rotate_vertical(Bigpatch,angle);

% Segment = rotate_vertical(Segment,angle);

% Crop down to deal with boundary problem after rotation.
Patch = Patch(crop_down1:end-crop_down1,crop_down1:end-crop_down1);
Bigpatch = Bigpatch(crop_down:end-crop_down,crop_down:end-crop_down);
